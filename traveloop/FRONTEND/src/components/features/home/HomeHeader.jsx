...
export default function HomeHeader({
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/70 dark:border-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">

        {/* Logo */}
        <div className="flex items-center gap-2.5 shrink-0 cursor-pointer" onClick={() => navigate('/home')}>
          <div className="w-9 h-9 rounded-xl bg-primary/10 dark:bg-primary/20 flex items-center justify-center">
            <Send size={18} className="text-primary" />
          </div>
          <span className="font-poppins text-lg font-extrabold text-primary tracking-tight hidden sm:inline">Traveloop</span>
        </div>
        ...
        <div className="relative min-w-0 flex-1 max-w-lg" ref={searchRef}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 rounded-xl py-2 pl-11 pr-16 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
            {!searchQuery && (
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-2 top-1/2 flex min-h-8 min-w-8 -translate-y-1/2 items-center justify-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
              >
                <X size={14} />
              </button>
            )}
          ...
          <AnimatePresence>
            {searchOpen && searchQuery.trim() && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -4 }}
                className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-slate-900 rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.15)] border border-slate-200/80 dark:border-slate-800 overflow-hidden z-50 max-h-80 overflow-y-auto"
              >
                {filteredTrips.length > 0 ? (
                  filteredTrips.map((t) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors text-left group"
                    >
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          <button
            ...
            className="hidden md:inline-flex items-center gap-1.5 bg-gradient-to-r from-primary to-accent text-white font-poppins text-xs font-bold px-3.5 py-2 rounded-xl shadow-sm hover:shadow-md hover:scale-105 active:scale-95 transition-all"
          >
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-primary transition-colors shrink-0 hidden sm:flex items-center justify-center"
          >
            <User size={18} />
          </button>
