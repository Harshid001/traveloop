...
export default function TopDestinationsCarousel({ 
  ...
  return (
    <section className="mb-12">
      <div className="flex items-end justify-between mb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="bg-primary/10 text-primary dark:bg-primary/20 p-2 rounded-xl">
              <Sparkles size={18} />
            </span>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs mt-1">Handpicked luxury & bucket-list experiences worldwide</p>
        </div>
      ...
      <div className="relative w-full overflow-hidden rounded-3xl shadow-hover group border border-slate-200/80 dark:border-slate-700/80 bg-slate-900">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="absolute top-5 right-5 z-20 flex items-center gap-3">
              <span className="bg-primary/95 backdrop-blur-md text-white text-xs font-extrabold rounded-full px-4 py-1.5 shadow-lg border border-white/20">
                {activeTrip.price}
              </span>
              <button
            ...
            <button
              ...
              onClick={handlePrev}
              className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white opacity-0 group-hover:opacity-100 flex items-center justify-center hover:bg-white hover:text-slate-900 hover:scale-110 transition-all shadow-xl"
            >
              <ChevronLeft size={22} />
            </button>
            <button
              ...
              onClick={handleNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white opacity-0 group-hover:opacity-100 flex items-center justify-center hover:bg-white hover:text-slate-900 hover:scale-110 transition-all shadow-xl"
            >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 z-10">
              <div className="max-w-3xl">
                ...
                <div className="flex items-center gap-4">
                  <button
                    onClick={() => navigate(`/trip/${activeTrip.id}`)}
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-primary via-secondary to-accent text-white font-poppins font-bold text-xs sm:text-sm px-6 py-3 rounded-xl shadow-lg hover:shadow-glow hover:scale-105 transition-all duration-300 group/btn"
                  >
