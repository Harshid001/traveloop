          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700/80">
              <div className="flex items-center gap-2.5">
                ...
                <h3 id="share-modal-title" className="font-poppins text-base font-bold text-slate-900 dark:text-slate-100">Share Trip</h3>
              </div>
              <button aria-label="Close share dialog" onClick={onClose} className="tap-target rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                {socials.map((s) => (
                  <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer"
                    ...
                    className="flex min-h-20 flex-col items-center gap-2 rounded-xl py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all duration-200 group">
              ...
              <div className="flex gap-2">
                <div className="min-h-11 flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-600 dark:text-slate-300 truncate flex items-center gap-2">
                  <ExternalLink size={13} className="text-slate-400 shrink-0" />
                  <span className="truncate">{shareUrl}</span>
                </div>
                <button onClick={copyLink}
                  className={`min-h-11 justify-center px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 flex items-center gap-1.5 ${
                    copied ? 'bg-success text-white' : 'bg-primary hover:bg-primary-dark text-white shadow-sm'
                  }`}>
