...
export default function TripDetailScreen() {
  ...
  const image = trip.image || 'https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=800&q=80';
  const location = trip.location || trip.destination || '';
  const rating = trip.rating || 4.5;
  const budget = trip.budget || trip.totalBudget || 3000;
  const description = trip.description || 'Explore this amazing destination with a carefully curated itinerary.';
  const duration = trip.duration || '5 Days / 4 Nights';
  const facilities = trip.facilities || ['Hotel', 'Flights', 'Meals', 'Guide', 'Insurance'];

  const budgetNum = typeof budget === 'number' ? budget : 3000;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            <p className="font-poppins text-xl font-bold text-primary mt-1">${typeof budget === 'number' ? budget.toLocaleString() : budget}</p>
          </div>
