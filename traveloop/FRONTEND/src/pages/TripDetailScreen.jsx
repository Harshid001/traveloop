...
import { useGetTripQuery } from '../services/apiSlice';
...
export default function TripDetailScreen() {
  ...
  const title = trip.title || `${trip.destination || 'Trip'} Getaway`;
  const image = trip.image || '';
  const location = trip.location || trip.destination || '';
  const rating = trip.rating || null;
  const budget = typeof trip.budget === 'number' ? trip.budget : (typeof trip.totalBudget === 'number' ? trip.totalBudget : null);
  ...
  const budgetBreakdown = budget
    ? [
        { label: 'Hotels', icon: Hotel, amount: Math.round(budget * 0.4), pct: 40, color: '#2563EB' },
        { label: 'Transport', icon: Car, amount: Math.round(budget * 0.25), pct: 25, color: '#F59E0B' },
        { label: 'Food', icon: Utensils, amount: Math.round(budget * 0.2), pct: 20, color: '#22C55E' },
        { label: 'Activities', icon: Ticket, amount: Math.round(budget * 0.15), pct: 15, color: '#4F46E5' },
      ]
    : [];
    ...
    <div className="min-h-screen bg-surface-50">
      <div className="relative h-[280px] sm:h-[320px] md:h-[380px] lg:h-[360px] max-h-[50vh]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
          ...
          {location && <p className="text-white/80 text-sm flex items-center gap-1"><MapPin size={14} /> {location}</p>}
        </div>
      </div>
        ...
        {budget !== null && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <p className="font-poppins text-xl font-bold text-primary mt-1">${budget.toLocaleString()}</p>
            </div>
            <div className="text-right">
              <p className="text-textMuted text-[10px] font-semibold uppercase tracking-widest">Duration</p>
              <p className="font-poppins text-sm font-bold text-textDark mt-1">{duration || 'N/A'}</p>
            </div>
          </motion.div>
        )}
