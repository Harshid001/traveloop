...
export default function HomePage() {
  ...
  const stats = [
    ...
    { label: 'Distance Travelled', value: distanceTravelled, icon: Globe, color: '#F59E0B', suffix: ' km', format: true, badge: '12 countries' },
    { label: 'Saved Destinations', value: 8, icon: Bookmark, color: '#EC4899', suffix: '', badge: '4 wishlist' },
  ];

  const quickNav = [
    { label: 'Create Trip', icon: Plus, route: '/create-trip', color: 'from-blue-600 to-indigo-600', badge: 'New' },
    { label: 'My Trips', icon: Map, route: '/my-trips', color: 'from-indigo-600 to-violet-600', count: '3' },
    { label: 'Itinerary', icon: Route, route: '/itinerary-builder', color: 'from-purple-600 to-pink-600', badge: 'AI' },
    ...
    { label: 'Packing', icon: ClipboardList, route: '/packing', color: 'from-rose-500 to-red-600' },
    { label: 'Journal', icon: StickyNote, route: '/journal', color: 'from-sky-500 to-blue-600' },
    { label: 'Saved', icon: Bookmark, route: '/saved', color: 'from-amber-600 to-yellow-600' },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F17] transition-colors duration-300">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-28 md:pb-16">
        {authNotice && (
          <div className="mb-6 rounded-2xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm font-semibold text-primary flex items-center justify-between">
            <span>{authNotice}</span>
            <span className="text-xs bg-primary/10 px-2.5 py-1 rounded-full">Logged In</span>
          </div>
        )}

        {/* Desktop Hero Welcome Section */}
        <section className="mb-10 relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white p-6 sm:p-10 shadow-hover border border-white/10 dark:border-slate-800">
          <div className="absolute -right-12 -bottom-12 w-96 h-96 bg-primary/20 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute left-1/3 -top-12 w-64 h-64 bg-accent/20 rounded-full blur-3xl pointer-events-none" />

          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-semibold text-blue-200 border border-white/15 mb-4">
              ...
              <h1 className="font-poppins text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white leading-tight">
                Welcome back, <span className="bg-gradient-to-r from-blue-400 via-indigo-200 to-pink-300 bg-clip-text text-transparent">{user.firstName}</span>! ✈️
              </h1>
              <p className="text-blue-100/80 text-sm sm:text-base mt-2 font-normal leading-relaxed max-w-xl">
              ...
              <div className="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-white/15 backdrop-blur-md p-2 rounded-2xl border border-white/20 shadow-xl max-w-xl">
                <input
                  type="text"
                  placeholder="✨ Ask Traveloop AI: 'Create 5-day Japan itinerary'..."
                  className="w-full bg-transparent px-4 py-2.5 text-sm text-white placeholder-blue-200/60 outline-none"
                  onKeyDown={(e) => {
                ...
                <button
                  ...
                  className="shrink-0 bg-gradient-to-r from-primary to-accent text-white font-poppins text-xs font-bold px-5 py-3 rounded-xl hover:shadow-glow hover:scale-105 active:scale-95 transition-all text-center"
                >
                  Generate Plan
                </button>
              </div>
            ...
            <div className="hidden lg:block shrink-0 w-80 bg-white/10 backdrop-blur-xl p-5 rounded-2xl border border-white/20 shadow-2xl">
              <div className="flex items-center justify-between text-xs font-bold text-blue-200 mb-2 uppercase tracking-wider">
                <span>Upcoming Adventure</span>
                <span className="bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-400/30">Confirmed</span>
              </div>
              <h3 className="font-poppins text-lg font-bold text-white leading-snug">Tokyo & Mt. Fuji Exploration</h3>
              <p className="text-xs text-blue-100/70 mt-1">Jun 15 — Jun 28, 2026 (14 Days)</p>

              <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between text-xs">
                <span className="text-blue-200">Pre-trip Packing</span>
          ...
          >
            {quickNav.map((item) => (
              <motion.button
                ...
                className="relative flex flex-col items-center justify-center text-center rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900/90 p-4 shadow-soft dark:shadow-card-dark transition-all duration-200 hover:shadow-hover hover:border-primary/30 dark:hover:border-primary/50 group cursor-pointer"
              >
                {item.badge && (
                ...
                {item.count && (
                  <span className="absolute top-2 right-2 text-[9px] font-extrabold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded-full">
                    {item.count}
                  </span>
                )}
                ...
                <span className="font-poppins text-xs font-bold text-slate-900 dark:text-slate-100 group-hover:text-primary transition-colors whitespace-nowrap">
                  {item.label}
                </span>
              </motion.button>
            ))}
