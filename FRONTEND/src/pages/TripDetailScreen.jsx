    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="relative h-[280px] sm:h-[320px] md:h-[380px] lg:h-[360px] max-h-[50vh]">
        ...
        <button aria-label="Go back" onClick={() => navigate(-1)}
          className="absolute top-5 left-5 z-10 tap-target rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center shadow-md hover:bg-white transition-colors">
          <ArrowLeft size={18} className="text-gray-700" />
        </button>
        <button aria-label={liked ? 'Remove from saved trips' : 'Save trip'} onClick={() => setLiked(!liked)}
          className="absolute top-5 right-5 z-10 tap-target rounded-full bg-white/80 backdrop-blur-md flex items-center justify-center shadow-md hover:scale-110 transition-transform">
          <Heart size={18} className={liked ? 'text-red-500 fill-red-500' : 'text-gray-500'} />
        </button>
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
          <div className="flex items-center gap-2 mb-2">
            <span className="inline-flex items-center gap-1 bg-black/20 backdrop-blur-md text-white text-xs font-medium rounded-full px-3 py-1">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          className="bg-white rounded-2xl p-5 shadow-soft border border-gray-100 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-gray-400 text-[10px] font-semibold uppercase tracking-widest">Budget</p>
            <p className="font-poppins text-xl font-bold text-accent mt-1">{budget}</p>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          <div className="flex flex-wrap gap-2">
            {facilities.map((f, i) => (
              <span key={i} className="inline-flex items-center gap-1.5 bg-[#EEF2FF] text-accent text-xs font-medium rounded-full px-3.5 py-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {activities.map((a, i) => (
                <div key={i} className="bg-white border border-gray-100 rounded-xl p-3 flex items-center gap-3 shadow-[0_1px_4px_rgba(0,0,0,0.03)] hover:shadow-md transition-shadow">
                  ...
                  <span className="text-xs font-bold text-accent shrink-0">${a.cost}</span>
                </div>
              ))}
            </div>
          </motion.div>
