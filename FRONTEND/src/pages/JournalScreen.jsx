      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-poppins text-lg font-bold text-slate-900">Trip Journal</h1>
              <p className="text-xs text-slate-400">{notes.length} notes</p>
            </div>
          </div>
          <button aria-label="Add note" onClick={startNew}
            className="tap-target rounded-full bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 hover:scale-105 active:scale-95 transition-all duration-300 shadow-sm hover:shadow-md">
            <Plus size={18} />
          </button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <AnimatePresence>
          {editing !== null && (
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}
              className="bg-white rounded-2xl border border-slate-200 shadow-[0_8px_30px_rgba(0,0,0,0.08)] p-5 mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-poppins text-sm font-bold text-slate-900">{editing === 'new' ? 'New Note' : 'Edit Note'}</h3>
                <button aria-label="Close note editor" onClick={() => setEditing(null)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full text-slate-400 hover:text-slate-600"><X size={16} /></button>
              </div>
              <div className="space-y-3">
                ...
                <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })}
                  ...
                  className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl py-3 px-4 text-sm text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all resize-none" />
                ...
                <button onClick={saveNote}
                  className="w-full bg-blue-600 text-white text-sm font-semibold rounded-full py-3 flex items-center justify-center gap-2 hover:bg-blue-700 active:scale-95 transition-all duration-300">
        ...
        {notes.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-slate-400 text-sm">No notes yet. Start journaling!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {notes.map((note, i) => (
              <motion.div key={note.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-[0_6px_20px_rgba(0,0,0,0.06)] transition-shadow group">
                {/* Color accent top */}
                <div className="h-1.5" style={{ backgroundColor: COLORS[note.color % COLORS.length] }} />
                <div className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <h4 className="font-poppins text-sm font-bold text-slate-900 leading-snug">{note.title}</h4>
                    <div className="flex gap-1 opacity-100 transition-opacity shrink-0 sm:opacity-0 sm:group-hover:opacity-100">
                      <button aria-label={`Edit ${note.title}`} onClick={() => startEdit(note)} className="tap-target rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors">
                        <PenLine size={12} />
                      </button>
                      <button aria-label={`Delete ${note.title}`} onClick={() => deleteNote(note.id)} className="tap-target rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 hover:text-red-400 transition-colors">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="text-[10px] text-slate-300 flex items-center gap-1"><Clock size={9} />{note.ts}</span>
                  </div>
                </div>
              </motion.div>
            ))}
