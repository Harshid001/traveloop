      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button aria-label="Notifications" onClick={() => navigate('/notifications')} className="tap-target hidden rounded-xl text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors shrink-0 sm:flex items-center justify-center"><Bell size={18} /></button>
            <button aria-label="Open profile page" onClick={() => navigate('/profile')} className="tap-target hidden rounded-xl text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors shrink-0 sm:flex items-center justify-center"><User size={18} /></button>
                  ...
                  <motion.div initial={{ opacity: 0, y: -6, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.96 }} transition={{ duration: 0.15 }}
                    ...
                    <div className="bg-gradient-to-br from-primary to-accent p-5">
                      <div className="flex items-center justify-between">
                        <div className="w-12 h-12 rounded-xl bg-white/20 border-2 border-white/40 flex items-center justify-center text-xl font-bold font-poppins text-white">{firstLetter}</div>
                    ...
                    {editMode ? (
                      <div className="p-4 space-y-3">
                        ...
                        <div className="flex gap-2 pt-1">
                          <button onClick={saveProfile} className="flex-1 bg-primary text-white text-xs font-semibold rounded-xl py-2.5 hover:bg-primary-dark transition-colors">Save</button>
                          <button onClick={() => { setEditMode(false); setEditForm(user); }} className="flex-1 border border-slate-200 text-textMuted text-xs font-semibold rounded-xl py-2.5 hover:bg-slate-50 transition-colors">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <div className="p-4 space-y-1">
                        <div className="flex min-w-0 items-center gap-3 text-textMuted text-sm py-2 px-2"><Phone size={15} className="shrink-0 text-slate-400" /><span className="truncate">{user.mobile}</span></div>
                        <div className="flex min-w-0 items-center gap-3 text-textMuted text-sm py-2 px-2"><Mail size={15} className="shrink-0 text-slate-400" /><span className="truncate">{user.email}</span></div>
                        <hr className="border-slate-100 my-2" />
                        <button onClick={handleLogout} className="w-full flex items-center gap-3 text-danger text-sm font-medium py-2.5 px-2 rounded-xl hover:bg-danger/5 transition-colors"><LogOut size={15} /> Log Out</button>
                      </div>
                    )}
                  </motion.div>
                )}
        ...
        <section className="mb-10">
          <div className="flex items-end justify-between mb-4">
            ...
            <div className="flex gap-1.5">
              {topTrips.map((_, idx) => (
                <button key={idx} aria-label={`Go to slide ${idx + 1}`} onClick={() => setCurrentSlide(idx)}
          ...
          <div className="relative w-full overflow-hidden rounded-2xl shadow-soft group hover:shadow-hover transition-shadow duration-300">
            <AnimatePresence mode="wait">
              <motion.div key={currentSlide} initial={{ opacity: 0, scale: 1.03 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
                ...
                <img 
                  src={topTrips[currentSlide].image} alt={topTrips[currentSlide].title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" 
                  loading="lazy"
                />
              ...
              >
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                    ...
                    <button onClick={() => navigate(`/trip/${trip.id}`)} className="text-primary text-sm font-bold hover:underline inline-flex items-center gap-1 group/btn">
